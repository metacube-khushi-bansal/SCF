...
import java.util.Scanner;
    ...
    public boolean isSymmetrical() {
        ...
        for (int[] element : elements) {
            ...
            for (int[] transposedElement : transposed.elements) {
                if (element[0] == transposedElement[0] && element[1] == transposedElement[1] && element[2] == transposedElement[2]) {
                    found = true;
                    break;
                }
            }
    ...
    public SparseMatrix add(SparseMatrix other) {
        ...
        while (i < this.elements.length && j < other.elements.length) {
            ...
            if (a[0] == b[0] && a[1] == b[1]) {
                int sum = a[2] + b[2];
                if (sum != 0) resultElements.add(new int[]{a[0], a[1], sum});
                i++;
                j++;
        ...
        for (int[] a : this.elements) {
            for (int[] b : transposedOther.elements) {
                if (a[1] == b[1]) {
                    ...
                    if (!found && value != 0) resultElements.add(new int[]{row, col, value});
                }
            }
        }
